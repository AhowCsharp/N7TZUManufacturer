...
export default function SalesAmountManage() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${API_PATH}/manufacturer/saleamount?start=${start.format('YYYY-MM-DD')}&end=${end.format('YYYY-MM-DD')}&pageNumber=${page+1}&pageSize=${rowsPerPage}`);
      ...
      if (response.status === 200) { 
        ...
        setFilterRows(response.data.source.orders);
